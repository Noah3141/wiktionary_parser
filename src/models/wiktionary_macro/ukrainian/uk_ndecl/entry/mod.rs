...
impl UkNDecl {
    pub fn get_form(&self, html: &scraper::Html, mut form_selector: &str) -> Option<String> {
        let singulare_tantum =
            self.check_head(html, "sg-only").expect("check_head") | self.macro_text.contains("sg");
        let plurale_tantum =
            self.check_head(html, "pl-only").expect("check_head") | self.macro_text.contains("pl");
        ...
        match form_selector {
            ...
            super::class_selectors::ACC_S | super::class_selectors::ACC_P | ".acc-form-of" => {
                ...
                if html
                    .select(&Selector::parse(".hypothetical").unwrap())
                    .next()
                    .is_some()
                {
                    // If the page balks and says the plural forms are *hypothetical, thus not having the expected classes, screw it, the form isn't recognized enough to keep
                    return None;
                }
            ...
            _ => {
                ...
                panic!(
                    "Failed to find a way to handle selection Some|None for: {form_selector} , {}",
                    &self.macro_text
                )
            }
        }
    ...
    pub fn gender(&self, html: &scraper::Html) -> Gender {
        ...
        if let true = self.check_head(&html, " fem ").expect("check_head attempt") {
            return Gender::Feminine;
        };
        if let true = self
