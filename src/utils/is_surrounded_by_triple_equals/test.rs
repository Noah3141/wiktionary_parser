...
#[test]
fn true_positive() {
    assert!(is_surrounded_by_triple_equals("===GKLjsl\0fkj фдлывао;sldkfjad osfijao fijasdpfoadfj==="))
}

#[test]
fn true_positive_despite_newline() {
    assert!(is_surrounded_by_triple_equals("\n\n===afaasdfak===  "))
}

#[test]
fn true_negative_due_to_only_double() {
    assert!(!is_surrounded_by_triple_equals("==False=="))
}
